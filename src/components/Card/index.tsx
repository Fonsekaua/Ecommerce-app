...
export function Index({ data, indice }: Props) {
    const [hoveredIndex, setHoveredIndex] = useState<number | null>(null)
    const { image, name, sale, discount, assessment, comments } = data;
    return (
        ...
        <div className="flex flex-col gap-1 shrink-0 ">
            <div className="bg-gray-200 cursor-pointer rounded-sm w-56 h-56 flex items-center justify-center relative" onMouseOver={() => setHoveredIndex(indice)} onMouseOut={() => setHoveredIndex(null)}>
                ...
                <div className="absolute w-full h-full ">
                    <div className={`w-full flex items-center justify-center bg-black absolute bottom-0 transition-all text-white ${hoveredIndex == indice ? "h-10 opacity-100" : "h-0 opacity-0 "} rounded-b-sm font-medium capitalize `} onClick={() => alert(name + " adicionado ao carrinho")}>
                        add to cart
                    </div>
                </div>
